1.安装依赖
首先，YanBot的所有版本都以Python编写，且KHB的Python版本>=3.10，OpenMariya的Python>=3.11且双方获取模块方式不一样，这里是KHB的方式。
安装pdm，输入以下字符安装pdm
Windows：pip install pdm
Linux：pip3 install pdm 或 pip3.11 install pdm
安装完毕后，进入YanBot的目录，输入pdm install（数据支持来自Bugji，也就是括弧）里面的numba和支持库因源已停止更新，且无需安装也能正常使用。所以打“X”也无需慌张。此刻你已经安装完毕了依赖。
...
这边注意下，cloud.json仅支持MySQL，用户也仅支持root；config.yaml的色图功能需在/modules/zh-cn/setu.py和/modules/zh-cht/setu.py这俩Python可执行文件删除即可关闭，且cloud.json的所有注释都必须删除。
配置结束，启动和使用YanBot
